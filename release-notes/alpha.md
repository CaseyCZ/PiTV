...
- **RPi4 Waydroid HW video decode je připravený jako vlastní vendor profil**: H.264/AVC používá `c2.v4l2.avc.decoder`; HEVC/H.265 používá `c2.ffmpeg.hevc.decoder` přes stateless `rpivid`/V4L2 Request API. PiTV přidává do Waydroidu chybějící `/dev/media*` passthrough, build zapisuje přesné upstream revize a instalace vendor image má automatický rollback při selhání kteréhokoli HALu. VP9/AV1 zůstávají software fallbacky.
...
- boot s vypnutou TV je podporovaný a první pozdější HDMI připojení projde stejnou recovery cestou; starý paralelní `pitv-display-watch`/`pitv-display.service` byl odstraněný
...
- **Aplikace** mají pevné menu: PiTV Store, Zobrazení aplikací, Odinstalovat aplikace a Obnovit seznam
- **Zobrazení aplikací** nejdřív otevře seznam aplikací a po výběru druhý seznam Zobrazit na ploše / Skrýt z plochy
- **Odinstalovat aplikace** má vlastní seznam bezpečně odinstalovatelných Linux i APK aplikací a až po výběru zobrazí potvrzení; odinstalace už není schovaná na pravé šipce
