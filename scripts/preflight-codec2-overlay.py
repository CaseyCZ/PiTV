...
if not stage.is_dir() or not target.is_dir(): raise SystemExit("stage/target root missing")
m=stage/"PITV-CODEC2-ROLLBACK.json"
if not m.is_file(): raise SystemExit("missing rollback manifest")
data=json.loads(m.read_text())
if data.get("version")!=1 or not isinstance(data.get("entries"),list): raise SystemExit("invalid rollback manifest schema")
report=[]
